...
export class ProteinCamelinaComponent implements OnInit {
  ...
  getUniprotData() {
    this.db.getDetailByUniprotid("camelina", encodeURIComponent(this.uniprotId)).subscribe({
      next: (res: any[]) => {
        ...
        if (this.arapidopsisData) {
          ...
          this.db.searchSpeciesMapper("camelina", encodeURIComponent(this.arapidopsisData.uniprot_id)).subscribe({
            next: (translation: any) => {
              this.translationObject = translation;
              this.proteinData = this.arapidopsisData;
              const geneNamesStr = this.proteinData?.gene_names ?? '';
              this.splitGeneNames = geneNamesStr.includes(',') 
                ? geneNamesStr.split(',').map((s: string) => s.trim()).filter((s: string | any[]) => s.length > 0)
                : geneNamesStr.split(' ').map((s: string) => s.trim()).filter((s: string | any[]) => s.length > 0);
